...
import { Search, Send, ChevronLeft, X, MessageCircle } from "lucide-react";
    ...
    fetch("http://localhost:5001/chat/rooms")
      ...
      .then((rooms) => {
        ...
        const convList = rooms.map((r: any) => {
          var otherUsers = (r.participants || []).filter(
            (p: string) => p !== currentUser,
          );
          var displayName =
            otherUsers.length > 0
              ? otherUsers.join(", ")
              : r.lastSender || "Unknown";
                ...
                messages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`max-w-xs lg:max-w-md xl:max-w-lg flex flex-col ${isMe ? "items-end" : "items-start"}`}
                      >
                        {!isMe && (
                        ...
                        <div
                          className={`px-4 py-2 rounded-2xl ${isMe ? "bg-primary text-primary-foreground rounded-br-md" : "bg-card border border-border rounded-bl-md"}`}
                        >
